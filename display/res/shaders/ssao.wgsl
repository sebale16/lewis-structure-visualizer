...
fn getViewPos(uv: vec2<f32>, dims: vec2<f32>) -> vec3<f32> {
    // what is depth value at this uv
    let depth = textureLoad(depthTexture, vec2<i32>(uv * dims), 0);
...
fn compute_main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let fragPos = getViewPos(uv, dims);
    let normal = textureLoad(normalTexture, vec2<i32>(id.xy), 0).xyz;
    ...
    for (var i = 0; i < 64; i++) {
        ...
        let actualPos = getViewPos(offsetUV, dims);
