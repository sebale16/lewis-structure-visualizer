...
void Molecule::FillMoleculeFromJSON(const std::string& jsonPath, const std::string& dataCSVPath) {
    ...
    for (const auto& [index, aEntry] : atomsEntries | std::views::enumerate) {
        // construct Atom
        Atom atom = Atom {
            .id = aEntry["id"],
            .globalId = static_cast<int>(index),
            .lone = aEntry["lone"],
...
std::expected<std::vector<BondedAtom>, std::string> Molecule::ComputeAtomLocsRots() {
    ...
    if (geometry == Geometry::Single) {
        ...
        if (std::ranges::empty(nonCentralAtomsFilter)) {
...
std::expected<std::vector<PiBond>, std::string> Molecule::ComputePiBondLocs(const std::vector<BondedAtom>& bondedAtoms, const std::vector<display::InstanceData>& pInstanceData) {
    ...
    for (int i = 0; i < atoms.size(); i++) {
        ...
        for (auto& piAtom : piBondedAtoms) {
            auto currAtom1Id = atoms[i]->globalId;
            auto currAtom2Id = piAtom.first.lock()->globalId;
